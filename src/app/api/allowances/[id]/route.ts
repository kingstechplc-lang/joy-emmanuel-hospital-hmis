...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await db.allowance.findUnique({
    where: { id },
    include: {
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  const session = await getSession();
  if (!session) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!hasPermission(session, PERMISSIONS.ALLOWANCE_MANAGE) && !hasPermission(session, PERMISSIONS.COMPENSATION_MANAGE) && !hasPermission(session, PERMISSIONS.PAYROLL_CREATE)) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }
  ...
  const allowedFields = [
    ...
    "isTaxable", "isRecurring", "profession", "active",
  ];
  ...
  const updated = await db.allowance.update({ where: { id }, data: updateData });
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.allowance.update({ where: { id }, data: { active: false } });
  await auditLog({
    userId: session.user.id,
    organizationId: session.user.organizationId,
    action: "ALLOWANCE_DEACTIVATED",
    resourceType: "allowance",
    resourceId: id,
  });
