...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await db.communityOutreach.findUnique({ where: { id } });
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await db.communityOutreach.findUnique({ where: { id } });
  ...
  const updated = await db.communityOutreach.update({
    where: { id },
    data: updateData,
  });
  ...
  await auditLog({
    ...
    action: "COMMUNITY_OUTREACH_UPDATED",
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.communityOutreach.delete({ where: { id } });
  await auditLog({
    userId: session.user.id,
    organizationId: session.user.organizationId,
    facilityId: existing.facilityId || undefined,
    action: "COMMUNITY_OUTREACH_DELETED",
    resourceType: "communityOutreach",
    resourceId: id,
  });
