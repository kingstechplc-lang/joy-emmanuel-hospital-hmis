// API: /api/coverage/[id] — GET / PATCH / DELETE
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await db.coverageRequest.findUnique({
    where: { id },
    include: {
      originalStaff: true,
      replacementStaff: true,
      facility: true,
      department: true,
      shift: true,
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updated = await db.coverageRequest.update({ where: { id }, data: updateData });
  await auditLog({ userId: session.user.id, organizationId: session.user.organizationId, action: "COVERAGE_UPDATED", resourceType: "coverage_request", resourceId: id, oldValues: existing, newValues: updateData });
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await db.coverageRequest.findUnique({ where: { id } });
  if (!existing || existing.organizationId !== session.user.organizationId) return NextResponse.json({ error: "Not found" }, { status: 404 });
  await db.coverageRequest.update({ where: { id }, data: { status: "cancelled" } });
  await auditLog({ userId: session.user.id, organizationId: session.user.organizationId, action: "COVERAGE_CANCELLED", resourceType: "coverage_request", resourceId: id });
