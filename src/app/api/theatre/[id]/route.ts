...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await db.theatreCase.findUnique({ where: { id } });
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await db.theatreCase.findUnique({ where: { id } });
  ...
  const updated = await db.theatreCase.update({
    where: { id },
    data: updateData,
  });
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.theatreCase.delete({ where: { id } });
  await auditLog({
    userId: session.user.id,
    organizationId: session.user.organizationId,
    facilityId: existing.facilityId || undefined,
    action: "THEATRE_CASE_DELETED",
    resourceType: "theatreCase",
    resourceId: id,
  });
