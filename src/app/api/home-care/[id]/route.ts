...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await db.homeCareVisit.findUnique({ where: { id } });
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await db.homeCareVisit.findUnique({ where: { id } });
  ...
  const updated = await db.homeCareVisit.update({
    where: { id },
    data: updateData,
  });
  ...
  await auditLog({
    ...
    action: "HOME_CARE_VISIT_UPDATED",
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.homeCareVisit.delete({ where: { id } });
  await auditLog({
    userId: session.user.id,
    organizationId: session.user.organizationId,
    facilityId: existing.facilityId || undefined,
    action: "HOME_CARE_VISIT_DELETED",
    resourceType: "homeCareVisit",
    resourceId: id,
  });
