// API: /api/training-assessments — GET (list) + POST (create)
...
export async function GET(req: Request) {
  ...
  const items = await db.trainingAssessment.findMany({
    where,
...
export async function POST(req: Request) {
  ...
  const item = await db.trainingAssessment.create({ data: body });
  await auditLog({ userId: session.user.id, organizationId: session.user.organizationId, action: "TRAINING_ASSESSMENTS_CREATED", resourceType: "trainingAssessment", resourceId: item.id, newValues: body });
