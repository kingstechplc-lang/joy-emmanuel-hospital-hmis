// API: /api/shift-swaps/[id]/cancel — POST (requester cancels)
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (existing.status === "approved" || existing.status === "completed") {
    return NextResponse.json({ error: "Cannot cancel an approved/completed swap. Contact supervisor." }, { status: 400 });
  }
  let body: any = {};
  ...
  const updated = await db.shiftSwap.update({
    ...
    data: {
      status: "cancelled",
      rejectionReason: body.reason || "Cancelled",
    },
  });
