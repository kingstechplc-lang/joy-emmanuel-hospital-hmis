...
export async function GET(req: Request) {
  ...
  if (type === "daily") {
    ...
    where.dischargedAt = { gte: start, lte: end };
  ...
  if (type === "monthly") {
    ...
    return NextResponse.json({ type, month: monthStr, items, count: items.length, byType, byDisposition });
  }
  ...
  if (type === "los") {
    ...
    const items = await db.dischargeRecord.findMany({
      where,
      select: { admissionDate: true, dischargedAt: true, dischargeType: true, patient: { select: { firstName: true, lastName: true, patientNumber: true } }, admission: { select: { admissionNumber: true, bedAssignments: { where: { status: "active" }, include: { ward: { select: { name: true } } }, take: 1 } } } },
      take: 500,
    });
    const losData = items.map((d) => {
  ...
  if (type === "by_type") {
    ...
    return NextResponse.json({ type, from: start, to: end, items: byTypeRaw.map((r) => ({ type: r.dischargeType || "routine", count: r._count })) });
  }
  ...
  if (type === "delayed") {
    ...
    const items = await db.dischargeRecord.findMany({
      ...
      include: { patient: { select: { firstName: true, lastName: true, patientNumber: true } }, admission: { select: { admissionNumber: true, bedAssignments: { where: { status: "active" }, include: { ward: { select: { name: true } }, bed: { select: { bedNumber: true } } }, take: 1 } } } },
    });
  ...
  if (type === "audit") {
    ...
    const logs = await db.auditLog.findMany({
      where: { resourceType: "discharge_record", createdAt: { gte: start, lte: end }, ...(facilityId ? { facilityId } : {}) },
