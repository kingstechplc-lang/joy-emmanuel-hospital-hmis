// =====================================================================
// API: /api/lab-tests/units
//   GET  — list units for the org
...
export async function GET(req: Request) {
  ...
  const items = await db.labTestUnit.findMany({
...
export async function POST(req: Request) {
  ...
  try {
    const item = await db.labTestUnit.create({
      data: {
    ...
    await auditLog({
      ...
      action: "LABTestUnit_CREATED", resourceType: "lab_test_unit", resourceId: item.id,
      newValues: { name, code },
    });
    return NextResponse.json({ item }, { status: 201 });
  } catch (e: any) {
    if (e?.code === "P2002") return NextResponse.json({ error: "Unit with this name or code already exists" }, { status: 409 });
    throw e;
  }
}
