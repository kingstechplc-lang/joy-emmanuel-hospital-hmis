// =====================================================================
// API: /api/lab-tests/categories
//   GET  — list categories for the org
...
export async function GET(req: Request) {
  ...
  const items = await db.labTestCategory.findMany({
...
export async function POST(req: Request) {
  ...
  try {
    const item = await db.labTestCategory.create({
      data: {
    ...
    await auditLog({
      ...
      action: "LABTestCategory_CREATED", resourceType: "lab_test_category", resourceId: item.id,
      newValues: { name, code },
    });
    return NextResponse.json({ item }, { status: 201 });
  } catch (e: any) {
    if (e?.code === "P2002") return NextResponse.json({ error: "Category with this name or code already exists" }, { status: 409 });
    throw e;
  }
}
