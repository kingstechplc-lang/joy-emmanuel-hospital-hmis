// API: /api/training-competencies — GET (list) + POST (create)
...
export async function GET(req: Request) {
  ...
  const items = await db.trainingCompetency.findMany({
    where,
    orderBy: { createdAt: "desc" },
    take: 500,
    include: { _count: { select: { staffCompetencies: true } } },
  });
...
export async function POST(req: Request) {
  ...
  const item = await db.trainingCompetency.create({ data: body });
  await auditLog({ userId: session.user.id, organizationId: session.user.organizationId, action: "TRAINING_COMPETENCIES_CREATED", resourceType: "trainingCompetency", resourceId: item.id, newValues: body });
