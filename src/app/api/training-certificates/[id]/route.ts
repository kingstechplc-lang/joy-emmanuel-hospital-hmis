// API: /api/training-certificates/[id] — GET / PATCH
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await db.trainingCertificate.findUnique({
    where: { id },
    include: {
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  delete updateData.certificateNumber; // don't allow changing cert number
  const updated = await db.trainingCertificate.update({ where: { id }, data: updateData });
  await auditLog({ userId: session.user.id, organizationId: session.user.organizationId, action: "TRAINING_CERTIFICATE_UPDATED", resourceType: "training_certificate", resourceId: id, oldValues: existing, newValues: updateData });
