  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle className="flex items-center gap-2"><HeartPulse className="w-5 h-5 text-rose-600" /> Register Blood Donor</DialogTitle></DialogHeader>
        ...
        <DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button onClick={() => onSubmit(form)} disabled={loading || !form.fullName}>{loading ? "Registering..." : "Register Donor"}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
...
function DonorDetail({ donor, onClose }: { donor: any; onClose: () => void }) {
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><HeartPulse className="w-5 h-5 text-rose-600" /> {donor.fullName}</DialogTitle>
          <DialogDescription>{donor.donorNumber}</DialogDescription>
        </DialogHeader>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle className="flex items-center gap-2"><Droplet className="w-5 h-5 text-rose-600" /> Register Blood Unit</DialogTitle></DialogHeader>
        ...
        <DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button onClick={() => onSubmit(form)} disabled={loading || !form.donorId || !form.expiryDate}>{loading ? "Registering..." : "Register Unit"}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
